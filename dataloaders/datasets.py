...
class DatasetTrain(Dataset):
    ...
    def check_databases(self, args):
        ...
        if (
            os.path.isdir(self.train_db_path)
            and len(os.listdir(self.train_db_path)) > 0
        ):
            pass
        else:
            if not os.path.isdir(self.train_db_path):
                ...
                DatasetTrain.GenerateVimeoHDF5Dataset(
        ...
        if (
            os.path.isdir(self.valid_db_path)
            and len(os.listdir(self.valid_db_path)) > 0
        ):
            pass
        else:
            ...
            if os.path.isfile(seq_list):
                with open(seq_list, "r") as f_sequences:
                    ...
                    for (
                        ...
                        shard_id,
                        _,
                    ) in zip(seq_shards, range(shards), tqdm(seq_shards)):
                        with h5py.File(
                            os.path.join(dst_path, "shard_%5.5d.h5" % shard_id), "w"
                        ) as fh5:
                            for seq, seq_id, _ in zip(
                                seq_shard, range(len(seq_shard)), tqdm(seq_shard)
                            ):
                                seq = seq.rstrip("\r\n")
                                ...
                                for img_name in images:
                                    img_list.append(
                                        Image.open(
                                            os.path.join(seq_path, img_name)
                                        ).convert("RGB")
                                    )
                                ...
                                fh5.create_dataset(
                                    sample_name,
                                    (len(img_list), sample_h, sample_w, 3),
                                    np.uint8,
                                )
    ...
    def __getitem__(self, index):
        ...
        sample = self.ReadVimeoHDF5(
            file_path=os.path.join(self.train_db_path, self.shard_list[shard_id]),
            key=sample_id,
        )
...
class DatasetValid(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = self.ReadVimeoHDF5(
            file_path=os.path.join(self.valid_db_path, self.shard_list[shard_id]),
            key=sample_id,
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--db-h5-path", type=str, required=True, default=r"/mnt/d/vimeo_septuplet_h5"
    )
...
if __name__ == "__main__":
    ...
    DatasetTrain.GenerateVimeoHDF5Dataset(
        db_path=args.db_src_path,
        seq_list=seq_list,
        shards=150,
        dst_path=args.db_h5_path,
    )
